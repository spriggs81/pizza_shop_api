...
// Orders - Put route
// Required Data: ordierId && (size || dough || meat || nonMeat)
// Optional Data: size,dough,meat,nonMeat
lib._orders.put = (data,cb)=>{
   // Grab token from header and verify the format
   const token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == 25 ? data.headers.token.trim() : false;
   if(token){
      // Get token data
      lib._tokens.checkToken(token,(err,tokenData)=>{
         if(!err && tokenData){
            // Grab OrderId and grab if sent size, dough, meat,nonMeat and verify format
            const orderId = typeof(data.payload.orderId) == 'string' && data.payload.orderId.trim().length == 30 ? data.payload.orderId.trim() : false;
            const size = typeof(data.payload.size) == 'string' && _bot.checkSize(data.payload.size) == true ? data.payload.size.trim() : false;
            const dough = typeof(data.payload.dough) == 'string' && _bot.checkDough(data.payload.dogh) == true ? data.payload.dough.trim() : false;
            const meat = Array.isArray(data.payload.meat) == true && _bot.checkMeat(data.payload.meat) == true ? data.payload.meat : false;
            const nonMeat = Array.isArray(data.payload.nonMeat) == true && _bot.checkNonMeat(data.payload.nonMeat) == true ? data.payload.nonMeat : false;
            // Confirm if requirements are met
            console.log("orderid: ",orderId," / size: ",size," / dough: ",dough," / meat: ",meat," / nonMeat: ",nonMeat);
            if(orderId && (size || dough || meat || nonMeat)){
               // Grab order information
               _data.read('orders',orderId,(err,orderData)=>{
                  if(!err && orderData){
                     // Verify if order belongs to requester
                     if(tokenData.email == orderData.email){
                        if(size){
                           orderData.size = size;
                        }
                        if(dough){
                           orderData.dough = dough;
                        }
                        if(meat){
                           orderData.meat = meat;
                        }
                        if(nonMeat){
                           orderData.nonMeat = nonMeat;
                        }
                        orderData.updated = Date.now();
                        // Update existing order infomation and save changes
                        _data.update('orders',orderId,orderData,(err)=>{
                           if(!err){
                              cb(200,{"Sccess":"Order has been updated!"});
                           } else {
                              cb(500,{"Error":"Unable to update order infomation"});
                           }
                        });
                     } else {
                        cb(401,{"Error":"Unable to process request!"});
                     ...
                     cb(400,{"Error":"Unable to locate order infomation!"});
                  }
               });
            } else {
               cb(400,{"Error":"Requirements not met!"});
            }
         } else {
            cb(400,{"Error":"Unable to locate token or token is invalid!"});
         }
      });
   } else {
      cb(401,{"Error":"Missing or Invalid Token!"});
   }
}
...
// Required data: orderId
// Optional data: none
lib._orders.delete = (data,cb)=>{
   // Grab token and verify format
   const token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == 25 ? data.headers.token.trim() : false;
   if(token){
      // Grab token data if available
      lib._tokens.checkToken(token,(err,tokenData)=>{
         if(!err && tokenData){
            // Grab orderId from headers and verify format
            const orderId = typeof(data.queryString.orderId) == 'string' && data.queryString.orderId.trim().length == 30 ? data.queryString.orderId.trim() : false;
            if(orderId){
               // Grab order information
               _data.read('orders',orderId,(err,orderData)=>{
                  if(!err && orderData){
                     if(tokenData.email == orderData.email){
                        ...
                        _data.delete('orders',orderId,(err)=>{
                           if(!err){
                              ...
                              _data.read('users',_bot.hash(orderData.email.toLowerCase()),(err,userData)=>{
                                 if(!err && userData){
                                    const userOrders = typeof(userData.orders) == 'object' && userData.orders instanceof Array ? userData.orders : [];
                                    // Deleting order from user's Array and save changes
                                    const deletePoint = userOrders.indexOf(orderId);
                                    if(deletePoint > -1){
                                       userOrders.splice(deletePoint,1);
                                       _data.update('users',userData.id,userData,(err)=>{
                                          if(!err){
                                             cb(200,{"Success":"Order has been deleted"})
                                          } else {
                                             cb(500,{"Error":"Problem saving changes in user account!"});
                                          }
                                       });
                                    } else {
                                       cb(500,{"Error":"Unable able to locate order in client's account"})
                                    }
                                 } else {
                                    cb(500,{"Error":"Unable to locate user!"})
                                 }
                              });
                           } else {
                              cb(500,{"Error":"Unable to delete order!"});
                           }
                        });
                     } else {
                        cb(401,{"Error":"Unable to process request!"});
                     ...
                     cb(400,{"Error":"Unable to find Order infomation"});
                  }
               });
            } else {
               cb(400,{"Error":"Invalid OrderId!"});
            }
         } else {
            cb(401,{"Error":"Unable to locate toekn!"});
         }
      });
   } else {
      cb(401,{"Error":"Missing or Invalid Token!"});
